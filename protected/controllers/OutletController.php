...
class OutletController extends Controller
{
	...
	public function actionCreate()
	{
		...
		$model=new Outlet;
		$model->kd_outlet=mysql_real_escape_string($_POST['kd_outlet']);
		$model->nm_outlet=mysql_real_escape_string($_POST['nm_outlet']);
	...
	public function actionDelete()
	{
		...
		$id = mysql_real_escape_string(trim($_POST['kd_outlet']));
	...
	public function search()
	{
		...
		// search 
		$kd_outlet = isset($_POST['kd_outlet']) ? mysql_real_escape_string($_POST['kd_outlet']) : '';
		$nm_outlet = isset($_POST['nm_outlet']) ? mysql_real_escape_string($_POST['nm_outlet']) : '';
		...
		$sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'kd_outlet';
		...
		$criteria->addSearchCondition('kd_outlet',$kd_outlet);
		$criteria->addSearchCondition('nm_outlet',$nm_outlet);
		...
		$result['total'] = count(Outlet::model()->findAll($criteria));
		
		$criteria->offset=$offset;
		$criteria->limit=$rows;
		$criteria->order=$sort.' '.$order;
		
		foreach(Outlet::model()->findAll($criteria) as $data)
		{	
			$row[] = array(
				'id'=>$data->kd_outlet,
				'nm_outlet'=>$data->nm_outlet,
				'alamat'=>$data->alamat,
				'telp1'=>$data->telp1,
				'cp'=>$data->cp,
				'status'=>$data->status,
